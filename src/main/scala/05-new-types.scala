...
object intersection_types:
   ...
   */
  // IsEqual ...
  IsEqual[HasLogging & HasUserRepo, HasUserRepo & HasLogging] // Scala compiles this, so it considers the left type as the same on the right.
...
object opaque_types:
   ...
   */
  lazy val exampleEmail: Email = Email("fo")
  ...
  def printNatural(v: Natural): Unit = println(v.toString())
